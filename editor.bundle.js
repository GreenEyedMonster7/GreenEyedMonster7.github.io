    ...
    4124: (t, e, n) => {
        ...
        const TrackEditor = class {
            ...
            update(t) {
                ...
                get(this, editor_isActive, "f")) {
                    ...
                    window._cursor = get(this, editor_cursorGridPos, "f"),
        ...
        const EditorScene = class {
            constructor(t, e, n, s, o, a, r, h, l, c, d, g, f, p, u, m, v) {
                ...
                window._track = get(this, scene_track, "f"),
                (() => {
                ...
                (() => {
                    const track = get(this, scene_track, "f");
                    ...
                    style.textContent = `
                        ...
                        #heightmap-textarea::placeholder {
                        ...
                        #heightmap-skip:hover {
                            background: rgba(255,255,255,0.2);
                        }
                        #heightmap-import {
                            background: rgba(40, 80, 200, 0.8);
                        }
                        #heightmap-import:hover {
                            background: rgba(60, 100, 220, 0.9);
                        }
                        #heightmap-import:disabled {
                    ...
                    overlay.id = "heightmap-overlay";
                    const dialog = document.createElement("div");
                    dialog.id = "heightmap-dialog";
                    const title = document.createElement("h2");
                    title.textContent = "Heightmap Importer";
                    const subtitle = document.createElement("p");
                    subtitle.textContent = "Paste your scanner output below, or skip to start with an empty editor.";
                    const textarea = document.createElement("textarea");
                    textarea.id = "heightmap-textarea";
                    textarea.placeholder = "0,2,0,38,0,0,4,2,0,38,0,0,...";
                    const status = document.createElement("div");
                    status.id = "heightmap-status";
                    const buttons = document.createElement("div");
                    buttons.id = "heightmap-buttons";
                    const skipBtn = document.createElement("button");
                    skipBtn.id = "heightmap-skip";
                    skipBtn.textContent = "Skip";
                    const importBtn = document.createElement("button");
                    importBtn.id = "heightmap-import";
                    ...
                    importBtn.addEventListener("click", () => {
                        ...
                        if (!text) { status.textContent = "Please paste your scanner output first."; return; }
                        const values = text.replace(/\s+/g, '').split(',').map(Number);
                        if (values.length % 6 !== 0) {
                            status.textContent = `Warning: value count (${values.length}) is not a multiple of 6, truncating.`;
                        }
                        const parts = [];
                        const groupCount = Math.floor(values.length / 6);
                        for (let i = 0; i < groupCount; i++) {
                            const b = i * 6;
                            parts.push({ x: values[b], rotation: values[b+1], y: values[b+2], partId: values[b+3], z: values[b+4], color: values[b+5] });
                        }
                        status.textContent = `Parsed ${parts.length} parts. Importing...`;
                        importBtn.disabled = true;
                        skipBtn.disabled = true;
                        textarea.disabled = true;
                        track.clear();
                        const BATCH = 5000;
                        let i = 0, placed = 0, failed = 0;
                        const placeBatch = () => {
                            const end = Math.min(i + BATCH, parts.length);
                            for (; i < end; i++) {
                                const p = parts[i];
                                try { track.setPart(p.x, p.y, p.z, p.partId, p.rotation, 0, p.color, null, null); placed++; }
                                catch(e) { failed++; }
                            }
                            const pct = Math.round(i / parts.length * 100);
                            status.textContent = `Importing... ${pct}% (${i} / ${parts.length} parts)`;
                            if (i < parts.length) {
                                setTimeout(placeBatch, 0);
                            } else {
                                track.generateMeshes();
                                status.textContent = `Done! Placed: ${placed}${failed > 0 ? `, Failed: ${failed}` : ''}. Closing...`;
